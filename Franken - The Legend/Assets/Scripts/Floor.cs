...
public class Floor : MonoBehaviour 
{
	...
	{
        count++;
		posFloor += 3.973717f * (floor.GetComponent<BoxCollider2D>().size.x);
		posBG1 += 3.973717f * (bg1.GetComponent<BoxCollider2D>().size.x);
		posBG2 += 3.973717f * (bg2.GetComponent<BoxCollider2D>().size.x);
        //Debug.Log(count);
		if(count < floorValue){
